...
Deno allows you to pass your `.env` file with a `--env` argument 
```bash deno
deno run --allow-env --env index.ts
```
or a differently named env file via 
```bash deno
deno run --allow-env --env=.staging.env index.ts
```
You can also load it in-code with the `dotenv` library like so
```ts index.ts
